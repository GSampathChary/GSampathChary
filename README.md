# Hi, I'm Sampath Chary 👋

### AI Engineer | Backend Engineer | Applied Machine Learning

I’m an AI Engineer and Full Stack Developer with **1.10+ years of professional experience** building AI-enabled applications, backend services, and machine learning solutions.

Currently working at **ICAR – Indian Institute of Rice Research**, where I work on AI-powered agricultural applications involving **machine learning, computer vision, backend services, mobile applications, and production deployment**.

---

## 🧠 About Me

- 🤖 AI Engineer focused on practical AI applications
- 🐍 Strong interest in Python and AI/ML engineering
- ⚙️ Backend development using FastAPI and Spring Boot
- 🧠 Experience with TensorFlow/Keras and Computer Vision
- 📱 Flutter developer for AI-powered mobile applications
- 🗄️ PostgreSQL and REST API development
- 🌾 Working on AI solutions for agricultural research
- 🚀 Interested in building production-ready AI systems

---

## 🛠️ Tech Stack

### Programming
![Python]([https://www.python.org/](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white))
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![Dart](https://img.shields.io/badge/Dart-0175C2?style=flat&logo=dart&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-336791?style=flat&logo=postgresql&logoColor=white)

### AI / Machine Learning
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat&logo=tensorflow&logoColor=white)
![Keras](https://img.shields.io/badge/Keras-D00000?style=flat&logo=keras&logoColor=white)
![Scikit Learn](https://img.shields.io/badge/Scikit--Learn-F7931E?style=flat&logo=scikit-learn&logoColor=white)

**Computer Vision • Deep Learning • Image Classification • Transfer Learning • Data Augmentation • Model Evaluation • TensorFlow Lite**

### Backend
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat&logo=springboot&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat&logo=postgresql&logoColor=white)

**REST APIs • Microservices • Backend Integration • API Development**

### Application Development
![Flutter](https://img.shields.io/badge/Flutter-02569B?style=flat&logo=flutter&logoColor=white)

**Flutter • Dart • AI Application Integration**

### Tools
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=flat&logo=postman&logoColor=white)

---

## 🚀 Featured Projects

### 🌾 RAISE — Rice AI Stress Evaluator

AI-powered agricultural application developed at ICAR–Indian Institute of Rice Research for rice crop stress evaluation.

**Technologies:** Flutter • Python • FastAPI • Spring Boot • TensorFlow/Keras • TensorFlow Lite • PostgreSQL

📱 [Google Play Store](https://play.google.com/store/apps/details?id=com.iirr.raise)

---

### 📊 InsightBI AI – AI-Powered Business Intelligence & Analytics Platform

End-to-end business intelligence platform for sales analytics, customer segmentation, product performance, forecasting, data uploads, and natural-language AI insights.

**Technologies:** Next.js • React • TypeScript • FastAPI • Python • PostgreSQL • Power BI • Docker • Recharts • AI Copilot • ETL • RFM Analytics

🔗 [Live Demo](https://insight-bi-ai-beta.vercel.app/)

---

### 🏠 VistaraAI – PVC Interior Design Assistant

AI-powered full-stack platform for PVC kitchen cupboards, wardrobes, TV units, and custom interiors.

**Technologies:** Next.js • TypeScript • React • FastAPI • Python • LangChain • LangGraph • PostgreSQL • Docker

🔗 [Live Demo](https://vistara-ai-pvc-interior-studio-xi.vercel.app/)

---

### 🇮🇳 BharatEye – India GeoAI Intelligence & Earth Observation Platform

India-focused geospatial intelligence platform for satellite-data visualization, disaster monitoring, risk assessment, real-time intelligence events, and large-scale geospatial analytics.

**Technologies:** React • TypeScript • Mapbox GL JS • Deck.gl • FastAPI • Python • PostgreSQL • PostGIS • Redis • WebSocket • GeoJSON • GeoAI • Docker

🔗 [Live Demo](https://bharat-eye.vercel.app/)

---

### 🌾 RiceGPT AI

AI-powered conversational assistant focused on rice agriculture and crop-related queries.

**Technologies:** Python • FastAPI • AI/LLM Integration

🔗 [Live Demo](https://quiet-sun-da4e.gsampathchary454.workers.dev/)

---

### 📄 ResumeAI Pro

AI-powered resume analysis platform providing resume evaluation and improvement recommendations.

**Technologies:** Python • FastAPI • Generative AI

🔗 [Live Demo](https://resume-ai-pro-beryl.vercel.app/)

---

### 🎤 Interview Copilot AI

AI-powered interview preparation platform designed to assist candidates with technical interview preparation and response generation.

**Technologies:** Python • FastAPI • Generative AI

🔗 [Live Demo](https://interview-copilot-ai-ten.vercel.app/)

---

### 📊 DataInsight AI

AI-assisted data analysis platform for exploring datasets, generating visualizations, and extracting useful insights.

**Technologies:** Python • FastAPI • Data Analysis • Generative AI

🔗 [Live Demo](https://data-insight-ai-dywd.vercel.app/)

---

### 📈 AutoML Studio

Automated machine learning platform supporting dataset processing, model training, model comparison, and evaluation.

**Technologies:** Python • FastAPI • Scikit-learn • Machine Learning

🔗 [Live Demo](https://auto-ml-studio-lilac.vercel.app/)

---

## 💼 Professional Experience

### ICAR – Indian Institute of Rice Research

**Young Professional – I | AI & Full Stack Developer**

Working on AI-powered agricultural applications involving:

- Machine Learning and Computer Vision
- TensorFlow/Keras model development
- Python AI services
- FastAPI and Spring Boot backend services
- Flutter application development
- PostgreSQL
- AI model integration and deployment

---

## 🎯 Current Focus

```text
AI Engineering
      ↓
Applied Machine Learning
      ↓
Computer Vision
      ↓
Generative AI
      ↓
Python Backend Engineering
      ↓
Production AI Applications
